...
import trimesh
from robosuite.wrappers.gym_wrapper import GymWrapper
from gea.psl.env_text_plans import ROBOSUITE_PLANS
...
class RobosuitePSLEnv(PSLEnv):
    ...
    def compute_perpendicular_top_grasp(self, target_pos, quat):
    ...
    def compute_perpendicular_side_grasp(self, target_pos, quat):
        qpos, qvel = self.sim.data.qpos.copy(), self.sim.data.qvel.copy()
        orig_ee_quat = self.reset_ori.copy()
        ee_euler = mat2euler(quat2mat(orig_ee_quat))
        quat = self._eef_xquat
        obj_euler = mat2euler(quat2mat(quat))
        ee_euler[1] = obj_euler[1] + np.pi / 2
        target_quat1 = mat2quat(euler2mat(ee_euler)).copy()
        ...
        if self.use_sam_segmentation:
            ...
            if self.env_name == "Lift":
                ...
                if self.text_plan[self.curr_plan_stage][1] == "grasp":
                    ...
                    object_pos = self.pick_place_bin_locations[bin_num - 1].copy()
                    object_pos[2] += 0.125
                    if self.env_name == 'PickPlaceCerealMilk':
                        object_pos[2] += .025 # extra offset to avoid collision
                    object_quat = np.zeros(4)
                ...
                if "grasp" in self.text_plan[self.curr_plan_stage][1]:
                    ...
                    # extra offset to avoid collision
                    object_pos[2] += 0.15
                    object_pos[0] -= 0.065
    ...
    def get_target_pos(self):
        ...
        if self.estimate_orientation and self.env_name == 'Door':
            pos = (pos + np.array([0.0, self.vertical_displacement, -.1])).copy()
            quat = self.compute_perpendicular_side_grasp(pos, obj_quat)
        elif self.estimate_orientation and self.text_plan[self.curr_plan_stage][1] == "grasp": #self.num_high_level_steps % 2 == 0:
            pos = (pos + np.array([0.0, 0.0, self.vertical_displacement])).copy()
            quat = self.compute_perpendicular_top_grasp(pos, obj_quat)
        else:
            quat = self.reset_ori
        return pos, quat
    ...
    def named_check_object_grasp(self, obj_name):
        def check_object_grasp(*args, **kwargs):
            ...
            is_grasped = is_grasped and (pos[2] - initial_object_pos[2]) > 0.01
            return is_grasped
    ...
    def get_joint_bounds(self):
        jnt_range = self.sim.model.jnt_range[:7, :].copy().astype(np.float64)
        # expand the joint range by 10% to allow for some slack
        jnt_range[:, 0] -= 0.1 * np.abs(jnt_range[:, 0])
        jnt_range[:, 1] += 0.1 * np.abs(jnt_range[:, 1])
        return jnt_range
    ...
    def take_mp_step(self, state, is_grasped, state_idx, start, step, num_steps):
        if self.use_joint_space_mp:
            self.set_robot_based_on_joint_angles(
                state, self.reset_qpos,
                self.reset_qvel,
                obj_name=self.text_plan[self.curr_plan_stage - (self.curr_plan_stage % 2)][0],
                is_grasped=is_grasped
            )
            if state_idx < 10 and not is_grasped and self.curr_plan_stage > 0:
                # make the objects settle down
                for _ in range(1):
                    a = np.zeros(7)
                    a[-1] = -1
                    self._wrapped_env.step(a)
