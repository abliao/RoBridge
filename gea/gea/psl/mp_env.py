...
from gea.psl.vision_utils import reset_precompute_sam_poses
# from gea.psl.sam_utils import build_models
...
class PSLEnv(ProxyEnv):
    ...
    def get_curr_postcondition_function(self):
        ...
        elif self.text_plan[self.curr_plan_stage][1].lower() == "press":
            return self.named_check_object_placement(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "push":
            return self.named_check_object_push(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "pull":
            return self.named_check_object_pull(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "turn":
            return self.named_check_object_turn(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "close":
            return self.named_check_object_close(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "open":
            return self.named_check_object_open(self.text_plan[self.curr_plan_stage][0])
        elif self.text_plan[self.curr_plan_stage][1].lower() == "reach":
            return self.named_check_object_reach(self.text_plan[self.curr_plan_stage][0])
        else:
            raise NotImplementedError
    ...
    def step(self, action, get_intermediate_frames=False, **kwargs):
        ...
        if curr_plan_stage_finished: # advance plan to next stage using motion planner
            ...
            if self.text_plan[self.curr_plan_stage][1]!='push':
                if self.teleport_instead_of_mp:
    ...
    def mp_to_point(
        ...
        elif "press" in self.text_plan[self.curr_plan_stage][1]:
            is_grasped = self.named_check_object_grasp(self.text_plan[self.curr_plan_stage - 1][0])()
        elif "push" in self.text_plan[self.curr_plan_stage][1]:
            is_grasped = self.named_check_object_grasp(self.text_plan[self.curr_plan_stage - 1][0])()
        elif "grasp" in self.text_plan[self.curr_plan_stage][1]:
        ...
        # print('target_pos',len(target_pos),target_pos)
        # print('og_qpos',len(og_qpos),og_qpos)
        # print('og_eef_xpos',len(og_eef_xpos),og_eef_xpos)
        try:
