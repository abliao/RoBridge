...
def main(cfg):
    ...
    from gea.video import TrainVideoRecorder, VideoRecorder, FrameRecorder
    ...
    class Workspace:
        def __init__(self, cfg):
            ...
            self.eval_video_recorder = FrameRecorder(
                self.work_dir if self.cfg.save_eval_video else None,
                output_size = None
            )
        ...
        def eval(self):
            ...
            eval_until_episode = utils.Until(len(self.eval_env.all_envs)*10)
            ...
            success_times = {}
            sample_weights = {}
            # assert self.cfg.num_eval_episodes % len(self.eval_env.all_envs) ==0
            for env in self.eval_env.all_envs:
                sample_weights[env] = 10 # self.cfg.num_eval_episodes // len(self.eval_env.all_envs)
                success_times[env] = 0
            while eval_until_episode(episode):
                print('episode',episode)
                env_sample_weights = self.eval_env.__getattr__('sample_weights')
                ...
                sample_weights[self.eval_env.env_name]-=1
                # self.video_recorder.record(self.eval_env)
                self.video_recorder.init(self.eval_env, enabled=True)
                self.eval_video_recorder.init(self.agent,time_step.observation, enabled=True)
                ep_infos = (
                    []
                ...
                while not time_step.last():
                    ...
                    with torch.no_grad(), utils.eval_mode(self.agent):
                        # time_step.observation['pixels'] = time_step.observation['origin']
                        action = self.agent.act(
                            time_step.observation, self.global_step, eval_mode=True
                        )
                    time_step = self.eval_env.step(action)
                    self.video_recorder.record(self.eval_env)
                    self.eval_video_recorder.record(self.agent,time_step.observation)
                    if self.cfg.has_success_metric:
                ...
                if self.cfg.has_success_metric:
                    ...
                    if current_episode_max_success > 0:
                        num_successes += 1
                        success_times[self.eval_env.env_name]+=1
                episode += 1
                ...
                if current_episode_max_success>0:
                    success = 'succeed'
                self.video_recorder.save(f"{self.global_frame}_{self.eval_env.env_name}_{success}_{episode}", step=self.global_frame)
                self.eval_video_recorder.save(f"{self.global_frame}_{self.eval_env.env_name}_{success}_{episode}", step=self.global_frame)
            ...
            print('mean_success_rate is ',mean_max_success/ episode)
