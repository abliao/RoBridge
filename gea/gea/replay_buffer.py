...
class ReplayBufferStorage:
    ...
    def add(self, time_step):
        for spec_name, spec in self._data_specs.items():
            ...
            assert (
                spec.shape == value.shape
            ), f"Shape of spec - {spec_name}  {spec.shape}, does not match the value shape - {value.shape}"
            assert (
                spec.dtype == value.dtype
            ), f"Dtype of spec - {spec_name} {spec.dtype}, does not match the value dtype - {value.dtype}"
