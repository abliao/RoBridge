...
from .classifier_free_guidance_pytorch import ModalConditioner, classifier_free_guidance
...
class FilmEfficientNet(nn.Module):
    def __init__(self,model_path='efficientnet-b3',output_dims=512):
        super().__init__()
...
class Tokenizer(nn.Module):
    def __init__(
        ...
        *,
        ...
        if use_conditioner:
            conditioner_klass = ModalConditioner
...
class Actor(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.action_net=nn.Sequential(
            nn.Linear(embed_dim, action_shape[0])
        )
    ...
    def forward(
        ...
        modal_emb = self.modal_encoder(obs,device=video.device)
        cond_drop_prob = default(cond_drop_prob, self.cond_drop_prob)
        frames, device = video.shape[1], video.device

        learned_tokens = self.tokenizer(video,modal_emb)
        
        learned_tokens = self.transformer(learned_tokens)
        
        pooled = reduce(learned_tokens, 'b fn d -> b d', 'mean')
