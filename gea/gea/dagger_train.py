...
def main(cfg):
    ...
    class Workspace:
        def __init__(self, cfg):
            ...
            # load expert 
            self.cfg.expert.agent.obs_shape = (6, 168, 168)
        ...
        def train(self):
            ...
            collect_data_time = 50000 // self.cfg.action_repeat
            if_collect = 1
            init_collect = 1
            buffers = []
            while train_until_step(self.global_step):
                # if time_step.last():
                if time_step.last() or (if_collect==0 and ((self._global_step+1)%100)==0):
                    ...
                    if if_collect:
                        # if max_success==1:
                        for expert_step in buffers:
                            self.replay_storage.add(expert_step)
                        buffers = []
                    # reset env
                    if not expert_act:
                ...
                if if_collect:
                    ...
                    with torch.no_grad():
                        expert_action = self.experts[self.train_env.env_name].act(
                            time_step.observation, self.global_step, eval_mode=True
                        )
                    time_step.observation['pixels'] = pixels
                    ...
                    # expert_action =  np.clip(action,-1,1)
                ...
                if if_collect:
                    ...
                    time_step = self.train_env.step(action)
                    time_step = time_step._replace(action=expert_action)
                if self.cfg.has_success_metric:
                    episode_reward += time_step.reward["reward"]
                    success = int(time_step.reward["success"])
                ...
                self._global_step += 1
                collect_data_time -= 1
                if collect_data_time==0:
                    collect_data_time = (70000 // self.cfg.action_repeat) if if_collect else (30000 // self.cfg.action_repeat)
                    if_collect ^= 1
                    init_collect = 0
                    buffers = []
