...
mlgrasp_train_args_kwargs = {
    key: dict(args=[],
              kwargs={
                  'task_id': list(ALL_V2_ENVIRONMENTS.keys()).index(key),
              })
    for key, _ in MLGrasp_V2['train'].items()
}

mlgrasp_test_args_kwargs = {
    key: dict(args=[],
              kwargs={'task_id': list(ALL_V2_ENVIRONMENTS.keys()).index(key)})
    for key, _ in MLGrasp_V2['test'].items()
}

MLGrasp_ARGS_KWARGS = dict(
    train=mlgrasp_train_args_kwargs,
    test=mlgrasp_test_args_kwargs,
)
...
mlv_train_args_kwargs = {
    key: dict(args=[],
              kwargs={
                  'task_id': list(ALL_V2_ENVIRONMENTS.keys()).index(key),
              })
    for key, _ in MLV_V2['train'].items()
}

mlv_test_args_kwargs = {
    key: dict(args=[],
              kwargs={'task_id': list(ALL_V2_ENVIRONMENTS.keys()).index(key)})
    for key, _ in MLV_V2['test'].items()
}

MLV_ARGS_KWARGS = dict(
    train=mlv_train_args_kwargs,
    test=mlv_test_args_kwargs,
)
