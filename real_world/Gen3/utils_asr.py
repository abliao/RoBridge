...
def record_windows():
    # 设置参数
    MIC_INDEX = 0  # 根据实际情况设置麦克风索引
    DURATION = 5  # 录音时长（秒）
    OUTPUT_FILE = "temp/speech_record.wav"  # 输出文件名
...
def record_auto(MIC_INDEX=1):
    ...
    # 导出wav音频文件
    output_path = 'temp/speech_record.pcm'
...
def get_access_token():
    ...
    params = {"grant_type": "client_credentials", "client_id": "yYbmx42XvLgNRYL5Gu5Xgucu", "client_secret": "VTpgAHH6TpX5EsGhJxxzARyU9HV07XPL"}
...
def speech_recognition(audio_path='temp/speech_record.wav'):
    print('开始语音识别')
    #转换
    frames = get_file_content_as_base64(audio_path)
    #获取字节数
    file_size = get_file_size_in_bytes(audio_path)
    ...
    "rate": 16000,
    "channel": 1,
    "cuid": "MHJCZGyVXyoaw6tSQJ5P2hq6K2fDujfz",
    "dev_pid": 80001,
    "speech": frames,
    "len": file_size,
    ...
    response = requests.request("POST", url="https://vop.baidu.com/pro_api", headers=headers, data=payload)
    response_data = response.json()
    if "result" in response_data:
        speech_result = response_data["result"]
        speech_result = speech_result[0].replace("‘", "'").replace("’", "'")
        print('语音识别结果：', speech_result)
        return speech_result
    else:
        print("语音识别失败，返回信息：", response_data)
        return None
